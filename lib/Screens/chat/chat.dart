...
class Chat extends StatelessWidget {
  ...
  Chat({Key key, @required this.peerId, @required this.peerName, @required this.currentId}) : super(key: key);
...
class ChatScreen extends StatefulWidget {
  ...
  ChatScreen({Key key, @required this.peerId, @required this.peerName, @required this.currentId}) : super(key: key);

  @override
  State createState() => ChatScreenState(peerId: peerId, peerName: peerName, currentId: currentId);
}

class ChatScreenState extends State<ChatScreen> {
  ChatScreenState({Key key, @required this.peerId, @required this.peerName, @required this.currentId});
  ...
  bool isLastMessageLeft(int index) {
    if ((index > 0 && listMessage != null && listMessage[index - 1].data()['idFrom'] == currentId) || index == 0) {
      // TODO: AQUÍ VA LA NOTIFICACIÓN _showNotificationWithDefaultSound();
      return true;
    } else {
      return false;
    }
  ...
  Future<bool> onBackPress() {
    if (isShowSticker) {
      ...
      FirebaseFirestore.instance.collection('user').doc(currentId).update({'chattingWith': null});
      Navigator.pop(context);
    }
